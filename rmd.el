...
(define-key markdown-mode-map markdownmode-prefix-key
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "[") 'md-reftex-citation)
    map))
...
;; 使用下面函数 避免rmd 文件中需使用注释行：[//]: # (\bibliography{bibfile}), 快捷键设置 M-n [
(defun md-reftex-citation ()
  (interactive)
  (let ((reftex-docstruct-symbol 'org--rds)
    ...
    (org-with-wide-buffer
	 ...
	 (setq bib (match-string 1)
	       org--rds (list (list 'bib bib))))))
    (ignore-errors (call-interactively 'reftex-citation))
    (message (concat "The Bibliography File: " bib))
    ))
